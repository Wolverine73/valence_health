...
| 14MAR2012 - EM Added ReportingType format for Adventist
+-----------------------------------------------------------------------HEADER*/
...
/*	%end;*/
	...
	%else %do;
		...
		%end;
		%else %if %QUPCASE("&client.")="ADVENTIST" 	%then %do; 
			if put(provid,$ReportingType.) in ("V");
		%end;
		run;
		...
		where &var. ge 1 and put(provid,$provyn.) = "Y" and source = "P" and provspec in (&rank1. &rank2. &rank3.)
			...
			%end;		
			%else %if %QUPCASE("&client.")="ADVENTIST" 	%then %do; 
				and put(provid,$ReportingType.) in ("V");
			%end;
			;
