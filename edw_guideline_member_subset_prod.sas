...
%macro edw_guideline_member_subset;
...
/*  				  ,ROUND (((CAST(@cdate as int) - CAST(dob as int))/365.23),1 ) as ager_current		  
				  ,ROUND (((CAST(@pdate as int) - CAST(dob as int))/365.23),1 ) as ager_prior	
*/


		%if %sysfunc(exist(cihold.gline_member_&client_id.)) %then %do;
		    proc sql;
		      connect to oledb(init_string=&cihold.);
		      execute ( 
		                drop table [cihold].[dbo].[gline_member_&client_id.]  
		              ) 
		      by oledb; 
		    quit;
		%end;

/*  work around until Robyn implements changes for 1.3 drop member_key */
/*proc sql;*/
...
/**/
/*				from [ciedw].[dbo].[V_ACTIVE_MEMBER]*/
...
proc sql;
		...
		select member_key, dob, sex, date_of_death, ager_current, ager_prior, created_on, max(nh_flag) as nh_fl,
             min(hspc_adm_dt) as hospice_dt
			  ...
			  into [cihold].[dbo].[gline_member_&client_id.]

			from #gline_member_temp
		group by member_key,dob,sex,date_of_death,ager_current,ager_prior,created_on

				 create index IX_ager_current on [cihold].[dbo].[gline_member_&client_id.] (ager_current)
				 create index IX_ager_prior on [cihold].[dbo].[gline_member_&client_id.] (ager_prior)
				 create index IX_dob on [cihold].[dbo].[gline_member_&client_id.] (dob)
				 create index IX_sex on [cihold].[dbo].[gline_member_&client_id.] (sex)
				 create index IX_key on [cihold].[dbo].[gline_member_&client_id.] (member_key)
...
/*data cistage.gline_member_&client_id.;*/
/*  format member_key 16. created_on mmddyy10.;*/
/*  set cihold.gline_member_&client_id.;*/
/**/
/*run;*/
