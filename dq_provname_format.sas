...
output out = dq_provname1 (drop = _TYPE_ _FREQ_ rename = (provname = _provname));
run;
...
------------------------------------------------------------------------SASDOC*;
proc sql;
CREATE TABLE dq_provnamecheck AS SELECT m.raw_lname, m.raw_fname, m.raw_npi, m.practiceid, m._provname, 
...
proc sql;
CREATE TABLE dq_provnpicheck AS SELECT 	m.raw_lname, m.raw_fname, m.raw_npi, m.provname_up, m.practiceid, m._provname,
...
data _null_;
 ...
 put "ERROR:          PracticeID: " practiceid;
 put "ERROR:                      ";
run;
