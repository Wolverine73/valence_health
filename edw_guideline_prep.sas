...
/*HEADER------------------------------------------------------------------------
|
| program:  edw_guideline_prep.sas
|
| location: M:\CI\programs\EDW
|
| purpose:  Run the formats and other prep for the guidelines from the EDW
|
| logic:    
|              
|
...
| history:  
|
| 15FEB2012 - Valence Health  - Clinical Integration  1.0.01
|             Original
| 15FEB2012 - LS create workflow #2 example
|
+-----------------------------------------------------------------------HEADER*/
...
/*%let sysparm=%str(sk_prcs_ctrl_id=1 wflow_exec_id=12345 sas_prgm_id=33 client_id=6  sas_mode=test );*/
%bpm_environment;
...
/* WORKFLOW #2: RUN Guidelines Query to vGuidelineInput & Formatting */
%*let include = %str(M:\CI\programs\ValenceBaseMeasures\Retrospective\V3\TESTING\Release 1.0 Guidelines Query and Formatting.sas);  
%*include "&include."; 
