...
%macro edw_outlier_report;

libname current "M:\ci\sasdata\guidelines\client_%sysfunc(strip(&client_id.))\current";
libname prior "M:\ci\sasdata\guidelines\client_%sysfunc(strip(&client_id.))\prior";
...
proc sql;
  create table g_prior as
  select distinct guideline_name as guideline,submeasures_name,sum(patients) as eligible_prior,
        sum(compliance) as compliant_prior,(sum(compliance)/sum(patients)) as percentcompliant_prior
		from dmart.dmprv_guidelines
    where Period = 'Current' and Submeasures_name='Overall'
  group by guideline_name
...
proc sort data = out_det.guideline (where=(guidelinetype='V')) 
out = G_current (drop = eligible1 compliant1 percentcompliant1
...
data guideline_index (keep = guideline status eligible_prior eligible_current PercentCompliant_prior PercentCompliant_current gloutlier);
			...
			*SASDOC-------------------------------------------------------------------------------
			...
			proc sort data = guideline_index nodupkey;
			  by guideline;
			run;


			proc sort data = dmart.dmprv_guidelines  out = sub_prior1;
				  by guideline_name submeasures_name;
			...
			proc sql;
			  create table sub_prior1 as
			  select b.npi1 as pcpid,a.provider_key,a.guideline_name as guideline,a.submeasures_name as submeasure,a.patients as elig,
                a.compliance as comp
 			  from dmart.dmprv_guidelines a left outer join dmart.dmprv_provider b
  				on a.provider_key=b.provider_key
  				where period='Current'
			  order by guideline_name,submeasures_name
  				;
quit;
				...
				set ciedw.provider(keep=	provider_name clncl_int_eff_dt clncl_int_exp_dt npi1 provider_title
                       provider_key ci_status client_key);
				where client_key = &client_id.;

					length vlinkprovid 8. provtitle $10. provname $42. npi $10. adventid $4.;
					format prveffdt prvtermdt mmddyy10.;
					provname	= cats(provider_name);
					npi 	  	= upcase(cats(npi1));
					prveffdt	= datepart(clncl_int_eff_dt);
					prvtermdt	= datepart(clncl_int_exp_dt);
					provtitle   = cats(provider_title);
					vlinkprovid = provider_key;
			...
			proc sql;
			  create table gprov_prior2 as
			  select c.npi1 as pcpid,provider_name as pcpname,patients as eligible2,
                  compliance as compliant2,guideline_name as guideline,
			      (compliance/patients) as percentcompliant2
				  from dmart.dmprv_guidelines a left outer join dmart.dmprv_provider c
				  on a.provider_key = c.provider_key
				  where submeasures_name='Overall' and period='Current'
				  ;

			  create table gprov_prior1 as
			  select c.npi1 as pcpid,provider_name as pcpname,patients as eligible1,
                  compliance as compliant1,guideline_name as guideline,
			      (compliance/patients) as percentcompliant1
				  from dmart.dmprv_guidelines a left outer join dmart.dmprv_provider c
				  on a.provider_key = c.provider_key
				  where submeasures_name='Overall' and period='Prior'
				  ;

			  create table gprov_prior as
			  select a.pcpid,a.eligible2,a.compliant2,a.guideline,a.percentcompliant2,
			         b.eligible1,b.compliant1,b.percentcompliant1,a.pcpname
				  from gprov_prior2 a left outer join gprov_prior1 b
				  on a.pcpid=b.pcpid 
				  and a.guideline=b.guideline
				  and a.pcpname=b.pcpname
			  order by a.pcpid,a.guideline
				  ;
quit;


		proc sort data=current.g9 out=g9_curr;
			by guideline pcpid;
		run;

		proc sort data=prior.g9 out=g9_prior;
			by guideline pcpid;
		run;

		data guidelineprovider;
  			merge g9_curr (rename= (elig=eligible2 comp=compliant2 comprate=percentcompliant2))
     		g9_prior (rename= (elig=eligible1 comp=compliant1 comprate=percentcompliant1));
		...
		run;
				...
				proc sort data = guidelineprovider /*(where=(guidelinetype='V'))*/ out = Gprov_current;
				  by pcpid guideline;
				  run;
			...
			------------------------------------------------------------------------SASDOC*;
				...
				merge Gprov_current (rename = (Eligible2=eligible_cur Compliant2=compliant_cur percentcompliant2=percom_cur) 
					  ...
					  Gprov_prior (rename = (Eligible2=eligible_pri Compliant2=compliant_pri percentcompliant2=percom_pri) 
								   drop = Eligible1 Compliant1 pcpname percentcompliant1
	...
	*SASDOC--------------------------------------------------------------------------
	...
	------------------------------------------------------------------------SASDOC*;
		...
		length measure $80. total added dropped flagged 8.;
	...
	*SASDOC--------------------------------------------------------------------------
	...
	%gline;
		...
		run;
			...
			%let client=%qcmpres(&client.);
			/*ods pdf file="\\fs\&client.\Reports\Data_Quality_Reports\&client._Outliers_&sysdate..pdf" notoc; */
			ods pdf file="\\fs\cccpp\reports\Guidelines\%sysfunc(strip(&client.))_outliers_&sysdate..pdf" notoc;
			%let emailfile=%str(\\fs\cccpp\reports\Guidelines\%sysfunc(strip(&client.))_Outliers_&sysdate..txt);
			%let filename =%str(\\fs\cccpp\reports\Guidelines\%sysfunc(strip(&client.))_Outliers_&sysdate..pdf);
...
delete gl1 gl2 gl3 gl4 gl5 gl6 gl7 gl8 gl9 gl10 gl11 gl12 gl13 gl14 gl15 gl16 gl17 gl18 gl19 gl20
       gl21 gl22 gl23 gl24 gl25 gl26 gl27 gl28 gl29 gl30 gl31 glprovider gl_provs;
run;
quit;
