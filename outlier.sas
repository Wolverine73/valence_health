%macro outlier (client);
	%macro lib;
		%if "&client" = "NSAP" %then %do;
			...
			call symput("portal","\\ebicompute\Projects\NSAP\data");
		...
		%else %if "&client" = "PHS" %then %do;
			...
			call symput("portal","\\ebicompute\Projects\PHS\data");
...
/*			call symput("outlier","m:\PHS\sasdata\Portal\PortalOut\MonthlyComparison");	*/
			call symput("portal","\\ebicompute\Projects\Adventist\data");			*Prior guidelines: guideline, submeasures_current, guidelineprovider;
/*			call symput("portal","m:\Adventist\SASTemp\CIProcess\test");	*Prior guidelines: guideline, submeasures_current, guidelineprovider;*/
		...
		%else %if "&client" = "StLukes" %then %do;
			...
			call symput("portal","\\ebicompute\Projects\StLukes\data");
			call symput("clientid",3);
			run;
			%end;
		%else %if "&client" = "Exempla" %then %do;
			data _null_;
			call symput("out_det","m:\Exempla\sasdata\Portal\PortalOut");
			call symput("prov","m:\Exempla\sasdata\CIETL\provider");
			call symput("current1","\\Exempla\StLukes\SASTEMP\Current");
			call symput("prior1","m:\Exempla\SASTEMP\Prior");
			call symput("provfmt","m:\Exempla\sasdata\CIETL\provider\Formats");
			call symput("outlier","m:\Exempla\sasdata\Portal\PortalOut\MonthlyComparison");
			call symput("portal","\\ebicompute\Projects\Exempla\data");
			call symput("clientid",8);
			run;
		%end;
		%else %if "&client" = "CCCPP" %then %do;
			data _null_;
			call symput("out_det","m:\cccpp\sastemp\Portal\PortalOut");
			call symput("prov","m:\cccpp\sasdata\CIETL\provider");
			call symput("current1","m:\cccpp\sastemp\CIprocess\Current");
			call symput("prior1","m:\cccpp\sastemp\CIprocess\Prior");
			call symput("provfmt","m:\cccpp\sasdata\CIETL\provider\formats");
			call symput("outlier","m:\cccpp\sastemp\Portal\PortalOut\MonthlyComparison");
			call symput("portal","\\ebicompute\Projects\CCCPP\data");
			call symput("clientid",6);
			run;
		%end;
		%else %if "&client" = "HolyRedeemer" %then %do;
			data _null_;
			call symput("out_det","m:\HolyRedeemer\sastemp\Portal\PortalOut");
			call symput("prov","m:\HolyRedeemer\sasdata\CIETL\provider");
			call symput("current1","m:\HolyRedeemer\sastemp\CIprocess\Current");
			call symput("prior1","m:\HolyRedeemer\sastemp\CIprocess\Prior");
			call symput("provfmt","m:\HolyRedeemer\sasdata\CIETL\provider\formats");
			call symput("outlier","m:\HolyRedeemer\sastemp\Portal\PortalOut\MonthlyComparison");
			call symput("portal","\\ebicompute\Projects\HolyRedeemer\data");
			call symput("clientid",14);
			run;
		%end;
		...
		libname glnames oledb 
		    init_string="Provider=SQLOLEDB.1;Integrated Security=SSPI;Data Source=CHISQL;Initial Catalog=SasBiWeb;"
		    preserve_tab_names=yes;
	...
	%lib;
		...
		proc format cntlin=provfmt.provspec;
		proc format cntlin=provfmt.provname;
		run;
		...
		%put FILE MONTH: &month6.;
			...
			*SASDOC--------------------------------------------------------------------------
			...
			------------------------------------------------------------------------SASDOC*;
		%if "&client" = "NSAP" %then %do;
				proc sort data = /*prior1.G10*/ portal.guideline(where=(guidelinetype='V')) out = G_prior (drop = eligible1 compliant1 percentcompliant1 
		...
		%end;
		%else %do;
				proc sort data = /*prior1.G10*/ portal.guideline out = G_prior (drop = eligible1 compliant1 percentcompliant1 
																				rename = (eligible2 = eligible_prior 
																						  compliant2 = compliant_prior PercentCompliant2 = PercentCompliant_prior
																						/*elig = eligible_prior*/));
				  by guideline;
	...
	%if "&client." = "Adventist" %then %do;
				...
				run;
/*				data prov (keep=provname npi prveffdt prvtermdt);*/
/*				set prov.universe(rename = (provider = provname provider_joined_practice_date = prveffdt provider_left_practice_date = prvtermdt));*/
/*				run;*/
	%end;
	%else %do;
				data prov (keep=provname npi prveffdt prvtermdt);
	...
	%end;
			...
			*SASDOC--------------------------------------------------------------------------
			...
			%end;
				...
				data prov_movement (drop =  provspec guideline pcpname guidelinetype 
											...
											percentcompliant2 compliant2 eligible2 
											/*comprate comp elig*/);
			...
			*SASDOC--------------------------------------------------------------------------
			...
			------------------------------------------------------------------------SASDOC*;
				...
				%if "&Client." = "CCCPP" %then %let drop1 = Eligible1 Compliant1 guidelinetype;
				%else %let drop1 = quartile Eligible1 Compliant1 guidelinetype;
	...
	*SASDOC--------------------------------------------------------------------------
	| Determine comments made in the guideline programs since the last time the 
	| guidelines were run
	------------------------------------------------------------------------SASDOC*;
	data guideline_key_name;
	  LENGTH FMTNAME $18. TYPE $1 label $50. start $15.;
	set glnames.CI_guidelines (keep = project _Guideline_Key guideline);
	where (upcase(project)=upcase("&client.")) and (index(_Guideline_Key,"."));
	  KEEP START LABEL TYPE FMTNAME ;
	  RETAIN FMTNAME 'guideline_key_name'  TYPE 'C';
	    start = _Guideline_Key;
		label = guideline;
		output;
	  if _n_ = 1 then do;
	   start = "other";
	...
	proc sort data=guideline_key_name nodupkey;
	by start;
	run;
	PROC FORMAT CNTLIN=guideline_key_name ;
	RUN;

	data comments;
	set out_det.comments_all;
	length guideline $50.;
	  guideline = put(guideline_key,$guideline_key_name.);
	  if guideline = "" then guideline = guideline_key;
	run;
	...
	*SASDOC--------------------------------------------------------------------------
	...
	------------------------------------------------------------------------SASDOC*;
		...
		%else %if "&client." = "PHS" %then %do;
			ods noresults; /* supress the report viewer window */
		...
		%end;
		%else %if "&client." = "Exempla" %then %do;
			ods pdf file="\\fs\Exempla\Reports\Monthly_Reports\Data_Load\Outliers\Exempla_Outliers_&sysdate..pdf" notoc;
			%let emailfile=%str(\\fs\Exempla\Reports\Monthly_Reports\Data_Load\Outliers\Exempla_Outliers_&sysdate..txt);
			%let filename =%str(\\fs\Exempla\Reports\Monthly_Reports\Data_Load\Outliers\Exempla_Outliers_&sysdate..pdf);
		%end;
		%else %if "&client." = "CCCPP" %then %do;
			ods pdf file="\\fs\cccpp\reports\Monthly_Reports\Data_Load\Outliers\CCCPP_Outliers_&sysdate..pdf" notoc;
			%let emailfile=%str(\\fs\cccpp\reports\Monthly_Reports\Data_Load\Outliers\CCCPP_Outliers_&sysdate..txt);
			%let filename =%str(\\fs\cccpp\reports\Monthly_Reports\Data_Load\Outliers\CCCPP_Outliers_&sysdate..pdf);
		%end;
		%else %if "&client." = "HolyRedeemer" %then %do;
			ods pdf file="\\fs\HolyRedeemer\reports\Monthly_Reports\Data_Load\Outliers\HolyRedeemer_Outliers_&sysdate..pdf" notoc;
			%let emailfile=%str(\\fs\HolyRedeemer\reports\Monthly_Reports\Data_Load\Outliers\HolyRedeemer_Outliers_&sysdate..txt);
			%let filename =%str(\\fs\HolyRedeemer\reports\Monthly_Reports\Data_Load\Outliers\HolyRedeemer_Outliers_&sysdate..pdf);
		%end;
...
/*			by guideline;*/
		...
		ods pdf startpage=now;
	*SASDOC--------------------------------------------------------------------------
	| Comments made in the guideline programs since the last time the guideline was run
	------------------------------------------------------------------------SASDOC*;
		PROC REPORT data=comments nowindows split='/'
		style(header)=[font_face="Arial" font_size=%scale(8)pt just=center background=H14E8679] 
		style(column)=[font_face="Arial"];

			title1 justify=center f="Arial" h=12pt "Most Recent Program Updates, by Guideline";
/*			title4 justify=center f="Arial" h=10pt "#byval(guideline)";*/
			options pageno=1;

			footnote1 justify=right f="Arial" h=10pt "&client"; 
			footnote2 justify=right f="Arial" h=10pt "Section VII, Page ~{thispage}"; 

			options orientation=portrait nodate nonumber leftmargin=0.5in rightmargin=0.5in topmargin=0.5in bottommargin=0.5in;			

/*			by guideline_key guideline;*/
			column  guideline_key guideline date text;
					define guideline_key / group left id 'Guideline Key' style=[FONT_SIZE=6pt];
					define guideline / group left id 'Guideline' style=[cellwidth=6cm] style=[FONT_SIZE=8pt];

					define date / 'Comment Date' style=[FONT_SIZE=6pt];
					define text / flow style=[FONT_SIZE=6pt] 'Comment';
			break after guideline / dol dul skip;
		run;
	...
	%if "&client." = "NSAP" %then %do;
		%email_parms(	em_to=&emailid.,
						em_cc=ngryfakis@valencehealth.com,
	...
	%else %if "&client." = "PHS" %then %do;
		%email_parms(	em_to=&emailid.,
/*						em_cc=isison@valencehealth.com,*/
			            em_subject=&client Outliers Report,
			            em_msg_file=%str(&emailfile.),
			            em_from=&emailid.
						);
...
/*						em_cc=isison@valencehealth.com,*/
	...
	%else %if "&client." = "StLukes" %then %do;
		%email_parms(	em_to=&emailid.,
						em_cc=tkalra@valencehealth.com,
			            em_subject=&client Outliers Report,
			            em_msg_file=%str(&emailfile.),
			            em_from=&emailid.
						);
...
/*						em_cc=kgregory@valencehealth.com,*/
	...
	%end;
		%else %if "&client." = "CCCPP" %then %do;
		%email_parms(	em_to=&emailid.,
						em_cc=dmurray@valencehealth.com,
			            em_subject=&client Outliers Report,
			            em_msg_file=%str(&emailfile.),
			            em_from=&emailid.
						);
