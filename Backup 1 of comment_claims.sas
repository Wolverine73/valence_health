...
%end;
%else %if %upcase(&client)=ADVENTIST  %then %do;
	LIBNAME memfmt "M:\&client.\sasdata\CIETL\Members";
%end;
%else %do;
	LIBNAME memfmt "M:\&client.\sasdata\CIETL\Member";
%end;

libname parm '\\sasbi\Projects\Tools\parms';
libname in3 "\\sasbi\Projects\&client.\data";
...
/*these formats are already created for AHN*/
%end;
...
LENGTH FMTNAME $10. TYPE $1. start $10. label $10.;
...
data compfmt (compress=yes keep = fmtname type start label);
...
proc print data=compfmt (obs=100);
...
set compliant2;
length 	dob test_date 4. memberid $9. provid $10.  sex $1. proccd $5. diag1 $6. source $1.
...
dob = input(put(memberid,$Member_dob.),mmddyy10.);
