...
/*HEADER------------------------------------------------------------------------
|
| program:  edw_current_run.sas
|
| location: M:\CI\programs\EDW
|
| purpose:  Run the current period for the guidelines
|
| logic:    
|              
|
...
/*%let sysparm=%str(sk_prcs_ctrl_id=1 wflow_exec_id=8 sas_prgm_id=76 client_id=6  sas_mode=prod );*/
%bpm_environment;
...
/**SASDOC--------------------------------------------------------------------------*/
/*| create the member subset table 						   */
/*|										   */
/*+------------------------------------------------------------------------SASDOC*; */
...
proc sql noprint;
  select formparmval as src_cnt format $20. into: src_record_cnt
  from out_det.resolved_macros_current
  where formparmname = "G0_COUNT"
  ;
 quit;
