...
/*      %let em_attach=%sysfunc(compress(&em_attach,%str(%')%str(%")));*/
      %let em_attach2=%sysfunc(compress(%bquote(&em_attach),%str(%')%str(%")' ,'));
		%PUT &EM_ATTACH;

      %let _attachments=;
		
      %do %while(%qscan(%qcmpres(%bquote(&em_attach2)),&i,%str( ))^=);

        %let _attach=%sysfunc(dequote(%qscan(%qcmpres(%bquote(&em_attach2)),&i,%str( ))));
		%put &_attach;
    ...
    filename mail_out email to="&em_to"
