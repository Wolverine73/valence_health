...
/*HEADER------------------------------------------------------------------------
|
| program:  edw_guideline_master_exempla.sas
|
...
|              
|
| input:    client_id   - the client id from vmine 
|      
...
|
+------------------------------------------------------------------------SASDOC*; 
/*EM 03/30/2012 - Not resolving in BPM environment, so need to resolve it here*/
...
/*** COPY DMPAT_COMMENT TABLE TO WORK FOLDER ***/
/*proc datasets; */
/*copy 	in = cistage */
/*		out = work; */
...
*SASDOC--------------------------------------------------------------------------
| Update the DM_XXXX.CI_CLIENT_PARAMETERS table with the run date 
|
+------------------------------------------------------------------------SASDOC*;
 
libname dmart  oledb init_string="Provider=SQLOLEDB.1;Integrated Security=SSPI;Data Source=CHISQL;
              Initial Catalog=&dmart.;" preserve_tab_names=yes insertbuff=10000 readbuff=10000;

proc sql;
  update dmart.CI_CLIENT_PARAMETERS
  set parameter_value = &rundt.
  where client_key = &client_id.
  and parameter_key=35;
quit;
