...
/*			if groupid not in ('9999999999','999999999999','NG99999999',''); /*** Valid Group Y/N - Map to Eligible Group ***/*/;
		...
		data latest_grpdt;	/*members latest service date with provider-group;*/
		  LENGTH FMTNAME $12. TYPE $1 start $25. label $36.;
		  set elig_group1;
		    KEEP START LABEL TYPE FMTNAME ;
		    RETAIN FMTNAME "latest_grpdt"  TYPE "C";
			by memberid descending &var. groupid descending svcdt;
				if first.&var.;
				memgroup = strip(strip(memberid) || "|" || strip(groupid));
				memgroupdt = strip(strip(memberid) || "|" || strip(groupid) || "|" || strip(put(svcdt,$mmddyy10.)));
				    start = memgroup;
					label = memgroupdt;
				output;
		  		if _n_ = 1 then do;
...
/*			&var. = 1;*/
		...
		run;
			...
			run;
				...
				data regroup;
				  LENGTH FMTNAME $7. TYPE $1 start $12. label $1.;
				  set bad_groups1;
				    KEEP START LABEL TYPE FMTNAME ;
			...
			run;
				...
				merge elig_ties2 (in = a)
					  elig_group4 (in = b where = (put(memberid,$regroup.) = "N"));
					by memberid;
				...
				data latest_groupid;	/**members seen most by 2 or more groups, need to run through tiebreaker;*/
				  LENGTH FMTNAME $14. TYPE $1 start $12. label $1.;
			...
			set elig_group4;
			where put(memberid,$latest_groupid.) = "N" and put(memberid,$regroup.) = "N";
			run;
...
/*		  &var. = 1;*/
		...
		run;
			...
			data latest_dt;	/*member's latest service date with provider-group;*/
			  LENGTH FMTNAME $9. TYPE $1 start $81. label $83.;
			  set elig_prov2;
			    KEEP START LABEL TYPE FMTNAME ;
			    RETAIN FMTNAME "latest_dt"  TYPE "C";
				by memberid descending &var. provgroup descending svcdt provid1;
					if first.&var.;
					memprovgroup = strip(strip(memberid) || "|" || strip(provgroup));
					    start = memprovgroup;
						label = provid1;
					output;
			  		if _n_ = 1 then do;
		...
		run;
			...
			run;
				...
				data dummy_first;	/**members with most seen, latest attributable physician is a dummy id;*/
				  LENGTH FMTNAME $11. TYPE $1 start $12. label $1.;
			...
			run;
				...
				data prov_tiebreaker5;	
/*				*all members NOT attributed to valid providerIDs, will move through guidelines with dummy provid and be attributed;*/
