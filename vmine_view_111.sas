...
/*HEADER------------------------------------------------------------------------
|
| program:  vmine_view_111
|
| location: M:\CI\programs\StandardMacros
|
| purpose:  Create practice data from CompulinkOA vmine view  
|
| logic:                   
|
| input:         
...
|			2. Filter logic has already been implemented in the new 
|			   standardized store procedure
|             
+-----------------------------------------------------------------------HEADER*/

%macro vmine_view_111;
...
/*	quit;*/
	...
	proc sql;
		connect to oledb(init_string=&emine.);
			create table practice_&do_practice_id. as select * from connection to oledb
			(	
				exec dbo.sp_CompulinkOA_Claims &do_practice_id., &maxprocessid.
			)
		;
	quit;
