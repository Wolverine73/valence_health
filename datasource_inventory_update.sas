/*HEADER------------------------------------------------------------------------
|
| program:  datasource_list_inventory_update.sas
|
...
%macro datasource_inventory_update ();
	...
	proc summary data=vmine_raw (keep=table) nway missing;
	class table;
	output out=tablecheck (drop=_type_ rename=_freq_=tblcnt);
	run;

	data tablecheck2;
	set tablecheck;
	if tblcnt in (.,0) then call symput('dataerrors',1);
