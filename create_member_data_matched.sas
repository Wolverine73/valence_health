...
/*HEADER------------------------------------------------------------------------
|
| PROGRAM:  create_member_data_matched.sas
|
| LOCATION: M:\CI\programs\StandardMacros 
|
| PURPOSE:  Create member dataset containing unique patient information after 
|           linking algorithim has been applied
| INPUT:    matchedvmineandpgf dataset
|
...
| HISTORY:  
|
| 26JULY2010 - Neema Thundathil  - Clinical Integration  1.0.01
|             Created member data table using matched data from linking algorithm
|  
|
+-----------------------------------------------------------------------HEADER*/

%macro create_member_data_matched;   

   
   *SASDOC--------------------------------------------------------------------------
   | Determine unique members by memberid, name and dob
   +------------------------------------------------------------------------SASDOC*;

	data member1 ;
	set dw.matchedvmineandpgf (keep=system practiceid memberid ssn lname fname dob sex svcdt address1 address2 city state zip phone);
	where ((lname NE "" and fname NE "" and dob NE .) or memberid ne "") and svcdt ge &startdate.;
	if substr(memberid,1,1) = 'S' then memberid = cats(substr(memberid,2)); 
	length ID2 $50.;
	dob_c=put(dob,DATE9.);
   ...
   *SASDOC--------------------------------------------------------------------------
   ...
   +------------------------------------------------------------------------SASDOC*;
	...
	Data dates2 (keep=memberid  sex firstdt lastdt address1 address2 city state zip phone system filename ssn);
		set dates1 ;
		by memberid svcdt;
...
%mend create_member_data_matched;
