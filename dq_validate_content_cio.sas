...
/*HEADER------------------------------------------------------------------------
|
| program:  dq_validate_content_cio.sas
|
...
|             Original
|
| 14JAN2011 - Robyn Stellman - update to include PGF files
|             
|
|             
+-----------------------------------------------------------------------HEADER*/

%macro dq_validate_content_cio(datain=, validate=);
  ...
  %if &validate = variables %then %do;
	...
	*--------------------------------------------------------------------------------
	...
	+------------------------------------------------------------------------------*;	         
	%let assessvariables=MEMBERID NPI SVCDT DIAG1 PROCCD FNAME LNAME DOB SEX PHONE ADDRESS1 CITY STATE ZIP POS;
  ...
  %else %if &validate = filename %then %do;
	...
	%if &filename_cnt. ne 0 and &pgf_practice eq %then %do;
	...
	%end;
	%else %if &filename_cnt. = 0 and &pgf_practice eq  %then %do;
	...
	%end;	
	%else %if &filename_cnt. ne 0 and &pgf_practice ne %then %do;
	...
	run;
		...
		data determine_date;
		  set filename;
		  format dateentered timestart mmddyy10.;
		  file=filename; 
		  datepart=compress(file,'0123456789',"k");
		  if length(datepart)=6 then datepart=left(trim(substr(datepart,1,2)))||"01"||left(trim(substr(datepart,3)));
		  dateentered=input(datepart,mmddyy8.);
		  timeend=date();
		  timeendc=compress(timeend);
		  timeendn=put(timeend,date9.);
		  timestart=intnx('month1.1',dateentered,-2)+14;
	...
	proc sort data=determine_date;
	   by descending dateentered timeendn;
	run;

	data determine_date;
	  set determine_date (obs=1);
	  by descending dateentered;
	  if first.dateentered;
	  filename_where="(where=(filename='"||left(trim(filename))||"'))";
	  call symput('filename_where',trim(filename_where));
	  call symput('filename',trim(filename));
        call symput('filename_last',trim(filename));
   run;
