/*HEADER------------------------------------------------------------------------
|
| program:  vmine_view_TRAKNET
|
| location: M:\CI\programs\StandardMacros
|
| purpose:  Create practice data from TRAKnet vmine view  
|
| logic:                   
|
| input:         
...
/*	quit;*/
	...
	proc sql;
		connect to oledb(init_string=&emine.);
			create table practice_&do_practice_id. as select * from connection to oledb
			(	
				exec dbo.sp_TRAKnet_Claims &do_practice_id., &maxprocessid.
			)
		;
	quit;
