...
/*HEADER------------------------------------------------------------------------
|
| program:  last_file_check_SB.sas
|
...
%macro last_file_check_SB (client);
	...
	create table tbl_cimaster2 as select * from connection to oledb
	(	
	  select table_name
	...
	quit;
/* recode for AmericanMedicalSoftware  */
    data tbl_cimaster;
	 format system $32.;
	set tbl_cimaster2;
     system=scan(table_name,1,'_');
	 if system='AMS' then system='AmericanMedicalSoftware';
/*	 if system='eClinicalWorks' then system='eClinical';*/
    run;

	proc sql;
	 create table pmsystem as
	  select distinct a.system
       from lastfilecheck3 a
	    join tbl_cimaster b
	   on a.system=b.system
       where a.system is not null;
	 quit;
  %let pmsystemnb= &sqlobs;
  %put &pmsystemnb;
  ...
  %do  icount=1 %to &pmsystemnb;
   data _Null_;
   set pmsystem;
    if _N_=&icount then call symput('system',strip(system));
	run;
   %put &system;
...
%vmine_pm_tbl_kProcessIDs (like_statement=&system.);
       ...
       %if &icount =1 %then %do;
         ...
         data sql_kprocessids2;
	     set sql_kprocessids;
		 run;
		  %end;
		%else %do;
         data sql_kprocessids2;
	     set sql_kprocessids2 sql_kprocessids;
		 run;
		 %end;
...
ods results;
	...
	where client = "&client" and system ne '';
...
%mend last_file_check_SB;

/* %last_file_check_SB (client=PHS) */

/*%last_file_check_SB (client=NSAP);*/

/*%last_file_check_SB (client=Ohio Health Group);*/

%last_file_check_SB (client=CCCPP);
